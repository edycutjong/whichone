...
function usd(n: number) {
  const m = Math.abs(n);
  return m >= 1e6 ? `$${(m / 1e6).toFixed(1)}M` : m >= 1e3 ? `$${Math.round(m / 1e3)}K` : `$${Math.round(m)}`;
}
function int(n: number) {
  return n >= 1e6 ? (n / 1e6).toFixed(1) + "M" : n >= 1e3 ? Math.round(n / 1e3) + "K" : String(Math.round(n));
}
function age(d?: number) {
  return d == null ? "age unknown" : d >= 365 ? `${(d / 365).toFixed(1)}y old` : `${Math.round(d)}d old`;
}
...
export function Card({
  ...
  return (
    <article className={`card ${state}`} data-id={`${c.chain}:${c.address}`} aria-label={`${c.name} on ${c.chain}`}>
      <div className="top">
        {/* token logos are arbitrary Nansen CDN URLs — a 36 px decorative <img> beats next/image's remotePatterns allow-list + optimizer cost */}
      ...
      {explain && s && s.scorable && !s.unchecked && (
