...
describe("whichOnesReal()", () => {
  ...
  it("abstains with the exact copy when search is empty", async () => {
    const v = await whichOnesReal(
      fakeClient(() => ({ tokens: [], entities: [], total_results: 0 })),
      "XQZPLM",
    );
  ...
  it("abstains when every candidate is unlabelled and young", async () => {
    const c = fakeClient((ep) => {
      if (ep === "search/general")
        return searchTokens([
          { chain: "base", address: IMP },
          { chain: "solana", address: "S1" },
        ]);
  ...
  it("stablecoins: no impostor flags, header flag set", async () => {
    const c = fakeClient((ep, body) => {
      if (ep === "search/general")
        return searchTokens([
          { chain: "ethereum", address: "U1", symbol: "USDC", name: "USD Coin" },
          { chain: "sei", address: "U2", symbol: "USDC", name: "USD Coin" },
        ]);
      if (ep === "tgm/flow-intelligence")
        return String(body.token_address) === "U1" ? flowRow({ smart_trader_wallet_count: 50, exchange_net_flow_usd: 5e7 }) : flowRow({});
      if (ep === "tgm/token-information")
        return String(body.token_address) === "U1" ? infoRow({ holders: 3e6 }) : infoRow({ deployed: "2026-09-15 00:00:00", holders: 40 });
  ...
  it("a failed lookup degrades that candidate only; the query still resolves", async () => {
    const c = fakeClient((ep, body) => {
      if (ep === "search/general")
        return searchTokens([
          { chain: "ethereum", address: REAL },
          { chain: "base", address: OLD },
        ]);
  ...
  it("cap limits scored candidates; finalists gets holders calls", async () => {
    const c = fakeClient((ep) => {
      if (ep === "search/general")
        return searchTokens(Array.from({ length: 12 }, (_, i) => ({ chain: "ethereum", address: "0x" + String(i).padStart(2, "0").repeat(20) })));
...
describe("review fixes F3/F4 (2026-09-16)", () => {
  ...
  it("F3: a token-information failure alone still scores, flagged partial with age unknown", async () => {
    const c = fakeClient((ep, body) =>
      ep === "tgm/token-information" && String(body.token_address) === REAL ? new Response("boom", { status: 500 }) : pepeRoutes(ep, body),
    );
    const v = await whichOnesReal(c, "PEPE", { now: NOW });
    const real = v.ranked.find((s) => s.address === REAL)!;
  ...
  it("F4: the hash ignores volatile search fields (market cap, volume, rank) and error text", async () => {
    const volatile = (mc: number) => (ep: string, body: Record<string, unknown>) => {
      if (ep === "search/general") {
        const r = pepeRoutes(ep, body) as ReturnType<typeof searchTokens>;
        r.tokens = r.tokens.map((t) => ({ ...t, market_cap: mc, volume_24h: mc * 3, rank: t.rank! + 7 }));
        return r;
      }
      return pepeRoutes(ep, body);
    };
  ...
  it("F4: the hash changes when the winner or an impostor flag changes", async () => {
    ...
    const noImp = await whichOnesReal(
      fakeClient((ep, body) =>
        ep === "tgm/token-information" && String(body.token_address) === IMP
          ? infoRow({ deployed: "2020-01-01 00:00:00", holders: 900 })
          : pepeRoutes(ep, body),
      ),
      "PEPE",
      { now: NOW },
...
describe("review round 2 (2026-09-16)", () => {
  ...
  it("R2-3: the unchecked warning counts only checked candidates (perp rows excluded)", async () => {
    const c = fakeClient((ep, body) =>
      ep === "tgm/flow-intelligence" && String(body.token_address) === OLD ? new Response("x", { status: 503 }) : pepeRoutes(ep, body),
    );
    const v = await whichOnesReal(c, "PEPE", { now: NOW });
...
describe("holders tiebreak (fixture edge 8 — no live ticker produced a flow tie on 2026-09-16, so the flip is proven here)", () => {
  it("two candidates with identical flow facts: the one Nansen recognises more holders of wins, and the reason says so", async () => {
    ...
    const c = fakeClient((ep, body) => {
      ...
      if (ep === "tgm/holders")
        return holdersRows(body.token_address === B ? Array(15).fill("Token Millionaire").concat(Array(5).fill(null)) : [null, "Token Millionaire"]);
...
describe("onProgress (streams the web page's pending → scored → verdict beats)", () => {
  it("emits candidates once, one scored event per chosen candidate, then the verdict — same hash as the return value", async () => {
    const events: string[] = [];
    let last: unknown;
    const v = await whichOnesReal(fakeClient(pepeRoutes), "PEPE", {
      now: NOW,
      onProgress: (e) => {
        events.push(e.type);
        last = e;
...
describe("address pasted instead of a ticker (live QA 2026-09-16)", () => {
  it("abstains with a hint that says what to type instead", async () => {
    const c = fakeClient((ep) => {
      if (ep === "search/general") return searchTokens([]);
      throw new Error("unexpected " + ep);
    });
