...
for (const f of set) {
  ...
  console.log(
    `${f.query.padEnd(8)}${f.chain ? `--${f.chain} ` : ""}→ ${path}  ${verdict.candidatesTotal} cands · ${outcome} · ${verdict.credits} cr / ${live.length} calls / ${(verdict.ms / 1000).toFixed(1)}s · ${verdict.hash.slice(0, 12)}${failed.length ? `  ⚠ ${failed.length} call(s) failed: ${failed.map((c) => `${c.endpoint} ${c.body.chain}`).join(", ")}` : ""}`,
  );
}
