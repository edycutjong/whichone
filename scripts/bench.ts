...
for (const f of set) {
  const row: Row = {
    query: f.query + (f.chain ? ` --chain ${f.chain}` : ""),
    coldMs: [],
    warmMs: [],
    credits: [],
    calls: [],
  ...
  for (let i = 0; i < runs; i++) {
    ...
    row.failed += cold.provenance.filter((c) => !c.ok).length;
  ...
  rows.push(row);
  console.error(
    `${row.query.padEnd(20)} cold p50 ${pct(row.coldMs, 50)} ms · warm p50 ${pct(row.warmMs, 50)} ms · ${pct(row.credits, 50)} cr · ${row.winner}${row.failed ? ` · ${row.failed} failed calls` : ""}${row.hashStable ? "" : " · hash moved between runs"}`,
  );
}
...
for (const r of rows)
  console.log(
    `| ${r.query} | ${pct(r.coldMs, 50)} ms | ${pct(r.coldMs, 95)} ms | ${pct(r.warmMs, 50)} ms | ${pct(r.credits, 50)} | ${pct(r.calls, 50)} | ${r.failed} | ${r.winner} | ${r.hashStable ? "yes" : "no"} |`,
  );
console.log(
